...
:: 标签段  ------------------------- serverDaemon 启动成功提示
:start_P_OK
    echo   ServerDaemon 服务启动成功!
    goto :eof

:: 标签段  ------------------------- serverDaemon 启动错误提示
:start_P_ERROR
...
:: 标签段  ------------------------- 系统错误提示	缺少 tasklist.exe
:chk_port_1
    echo   # 缺少 %Sys32%\tasklist.exe, 无法进行. & %pause% & goto menu

:: 标签段  ------------------------- 系统错误提示	缺少 netstat.exe
:chk_port_2
...
:chk_port
    ...
    echo   正常开启 :
    echo   --------------------------------------------------------------------------
    echo      端口: %apache_port%       被Apache占用
    echo      端口: %database_port%     被MySQL/MariaDB占用
    echo      端口: %redis_port%     被Redis占用
    echo      端口: 5080     被Red5(prunsrv.exe)占用
    echo      端口: 88       被Socket(php.exe)占用
    echo      端口: 1238     被Socket(php.exe)占用
    echo      端口: 8360     被File Server (FileServer.exe)占用
    echo   --------------------------------------------------------------------------
